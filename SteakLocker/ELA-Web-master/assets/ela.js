...
( function( $ ) {
    ...
    $(document).ready(function() {
        var wh = $(window).outerHeight(), m=$('#main'), mh = m.outerHeight(), scr = m.data('scrollOffset'), fh=$('#footer').outerHeight();
        if (mh< wh) {
            m.find('.container_wrap_first').css({'min-height': (wh-scr-fh)+'px'});
        }
    });
    ...
    ELA.login = function(f, msg) {
        ...
        return ELA.ajaxFormSubmit(F, function(response) {
            ...
            if (status == 'ok') {
                ...
                ELA.formError(F, error ? error : 'An error occurred');
            }
        }, function(xhr, status, error) {
            ELA.formError(F, 'An error occurred.');
        });
        return false;
    };
