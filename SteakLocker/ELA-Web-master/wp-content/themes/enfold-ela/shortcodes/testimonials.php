...
if ( !class_exists( 'avia_sc_testimonial' ) )
{
    class avia_sc_testimonial extends aviaShortcodeTemplate
    {
        ...
        function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
        {
            ...
            $data = AviaHelper::create_data_string(['autoplay'=>$autoplay, 'interval'=>$interval, 'animation' => 'fade', 'hoverpause' => true]);
            $controls = false;
            ...
            $output .= '<div class="flex_column av_one_half  flex_column_div av-zero-column-padding first el_before_av_one_half  avia-builder-el-first">';

            $output .= ShortcodeHelper::avia_remove_autop($content, true);

            $output .= '</div><div class="flex_column av_one_half  flex_column_div av-zero-column-padding el_after_av_one_half avia-builder-el-last">';
            $output .= avia_sc_testimonial::$testimonial_images;
            $output .= '</div>';
        ...
        {
            extract(shortcode_atts(['src'=> "",  'name'=> "",  'subtitle'=> "",  'link'=> "", 'linktext'=>"", 'custom_markup' =>''], $atts, 'av_testimonial_single'));
            ...
            //avatar size filter
            $avatar_size = apply_filters('avf_testimonials_avatar_size', 'full', $src, $class);

            //avatar
            $markup = avia_markup_helper(array('context' => 'single_image','echo'=>false, 'custom_markup'=>$custom_markup));
            list($image_url) = wp_get_attachment_image_src( $src , $avatar_size , false);
            if($src) {
                $active = avia_sc_testimonial::$testimonial_images ? '' : 'active';
                $avatar  = "<div id='testimonial-{$src}' class='testimonial-{$src} {$active} avia-testimonial-image' $markup><img src=\"". $image_url ."\" /></div>";
            }
            ...
            avia_sc_testimonial::$testimonial_images .= $avatar;
