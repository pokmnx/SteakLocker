 ...
 */
        CGRect labelBounds = [self.titleLabel.text boundingRectWithSize:maximumLabelSize
...
*/
        ...
        attr = @{NSFontAttributeName: self.subtitleLabel.font};
        CGRect subtitleBounds = [self.subtitleLabel.text boundingRectWithSize:maximumLabelSize
                                                                options:options attributes:attr context:nil];
        
        CGFloat maxLabelWidth = MAX(ceilf(labelBounds.size.width),ceilf(subtitleBounds.size.width));
        ...
        if(subtitleBounds.size.height > 0){
            CGFloat totalHeight = labelBounds.size.height + subtitleBounds.size.height + marginY;
            CGFloat minY = (self.bounds.size.height / 2)  - (totalHeight / 2);
            
            CGFloat titleY = minY;
            self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, labelBounds.size.width, labelBounds.size.height));
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY + labelBounds.size.height + marginY, subtitleBounds.size.width, subtitleBounds.size.height));
        }else{
            CGFloat totalHeight = labelBounds.size.height;
            CGFloat minY = (self.bounds.size.height / 2)  - (totalHeight / 2);
